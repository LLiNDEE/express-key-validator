    ...
    Object.entries(expectedKeys).forEach(([param, v]) => {
        ...
        rules.forEach(ruleObject => {
            ...
            if(typeof incomingValue === 'undefined' || incomingValue.trim() === ""){
                ...
                return invalidParams.push({[param]: {'expected_type(s)': formattedRules}});
            }
            ...
            if(typeof ruleObject === 'object'){
                ...
                if(ruleObject.rule === 'email' && ruleObject?.regex !== ""){
                    if(!validateType[ruleObject.rule](incomingValue, ruleObject?.regex)){
                        if(exist) return;
                        isValid = false;
                        exist = true;
                        const formattedRules = formatRules(rules);
...
const formatRules = rules => {
    ...
    rules.forEach(ruleObject => {
        ...
        resolveInvalidMessage(ruleObject.rule, ruleObject.value) !== "" && formattedRules.push(resolveInvalidMessage(ruleObject.rule, ruleObject.value));
    })
...
: type === 'regex' ? `Did not match the regex: ${value}`
: ""
...
const checkMinValue = (value, minValue) => value && value.length >= minValue && value && minValue
const checkMaxValue = (value, maxValue) => value && value.length <= maxValue && value && maxValue
const checkEnum = (value, enums) => enums.includes(value)
const checkEmail = (email, regex) => email && email.includes("@") && email.includes('.') && email.length > 3 && email.match(regex ? regex : regExpEmail)
